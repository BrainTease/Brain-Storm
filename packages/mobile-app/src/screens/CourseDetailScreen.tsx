...
export function CourseDetailScreen() {
  ...
  useEffect(() => {
    apiClient.getCourse(route.params.courseId).then(setCourse).finally(() => setLoading(false));
  }, [route.params.courseId]);

  if (loading) return <View style={styles.center}><ActivityIndicator size="large" color={colors.primary} /></View>;
  if (!course) return <View style={styles.center}><Text style={styles.error}>Course not found</Text></View>;
